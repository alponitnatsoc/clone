...
class PrimaReminderCommand extends ContainerAwareCommand
{
    ...
    {
        $this
            ->setName('symplifica:prima:reminder')
            ->setDescription('Sends push notification and mail reminding payments of prima')
            ->setHelp('Sends push notification and mail reminding payments of prima')
        ;
    }

    protected function execute(InputInterface $input, OutputInterface $output)
    {
        $output->writeln('<comment>Running Cron Task PrimaReminder ' . date("Y/m/d h:i") . ' time_zone: ' . date_default_timezone_get() . '</comment>');
        $this->output = $output;

        $cronService = $this->getContainer()->get('app.symplifica_chrons');


        $response = $cronService->putPrimaReminderAction();
